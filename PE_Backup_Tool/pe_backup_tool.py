...
def inventory(root: Path, recursive: bool = True) -> Inventory:
    ...
    return Inventory(files, directories, time.perf_counter() - started,
                     root_stat.st_mtime_ns, root_stat.st_atime_ns)
...
def report(source_root: Path, target_root: Path, jobs: list[str], copy: bool) -> dict:
    ...
    started = time.perf_counter()
    rows = []
    root_has_files = any((path.is_file() or path.is_symlink()) and not is_ignored_file(path)
                         for path in source_root.iterdir())
    root_backup_has_files = any((path.is_file() or path.is_symlink()) and not is_ignored_file(path)
                                for path in target_root.iterdir())
    scopes = []
    if root_has_files or root_backup_has_files:
        scopes.append(("[ROOT FILES]", source_root, target_root, False))
    scopes.extend((job, source_root / job, target_root / job, True) for job in jobs)
    for job, source, target, recursive in scopes:
        left = inventory(source, recursive=recursive)
        copy_result = None
        if copy and recursive:
            copy_result = copy_job(source, target, left)
        right = inventory(target, recursive=recursive) if target.exists() else None
        verify_started = time.perf_counter()
        checked = compare(source, target, left, right)
        verification_seconds = time.perf_counter() - verify_started
        rows.append({"job": job, "scope": "job" if recursive else "root_files",
                     "source": str(source), "backup": str(target),
    ...
    rows.sort(key=lambda row: (row["scope"] != "root_files", -row["size_bytes"],
                               row["job"].casefold()))
    return {"created_at": datetime.now().astimezone().isoformat(timespec="seconds"),
...
<p>Generated: {html.escape(data['created_at'])}<br>Source: {html.escape(data['source_root'])}<br>
...
Files placed directly under the source root appear in the [ROOT FILES] row.</p><p>Python source scan: {data['source_scan_seconds']:.3f}s;
...
<table><thead><tr><th>Job</th><th>Files</th><th>GiB</th><th>Folder modified</th>
<th>Folder accessed</th><th>Latest file modified</th><th>Latest file accessed</th><th>Scanned at</th>
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--copy", action="store_true", help="Copy explicitly selected jobs; never delete or overwrite")
    parser.add_argument("--excel-seconds", type=float, help="Measured Excel scan time for same jobs and scope")
    args = parser.parse_args(argv)
    if args.copy and not args.jobs:
        parser.error("--copy requires at least one explicit --job")
    ...
    try:
        ...
        jobs = discover_jobs(source_root, args.jobs)
        data = report(source_root, backup_root, jobs, args.copy)
